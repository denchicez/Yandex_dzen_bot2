...
def parse_states(href):
    ...
    return scrolling_states(next_url,states_local_status,states_local_url,was_tried)
@dp.message_handler(commands=['start_back_programm_time'])
async def start_back_programm_time(message):
        ...
        while(True):
            ...
            if(time.time()-k>=60*30):
                ...
                for person in persons:
                    ...
                    if(answer!=''):
                        await bot.send_message(person, answer)
                ...
                for time_index in range(0,len(time_status)):
                    if(time_status[time_index]!=-1):
                        ...
                        if(time_status[time_index]==0):
                            ...
                            await bot.send_message(list(persons)[time_index], 'У вас истекла подписка. Каналы удалены')
############################################################
